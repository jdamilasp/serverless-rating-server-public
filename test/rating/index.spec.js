...
describe('rating', function () {
  ...
  /** TODO : Update and Delete Function Testing  */
  describe('updateRating', function () {
    ...
    it('updates rating successfully', async function () {
      const request = require('../fixture/update-rating-request.json');
      const searchResponse = require('../fixture/aws/dynamodb-query-item.json');
      const body = JSON.stringify(request.body);
      searchResponse.Items[0].userId.N = body.userId;
      searchResponse.Items[0].contentId.N = body.contentId;
    ...
    it('does not update rating when validation is not successful', async function () {
      const request = require('../fixture/create-rating-invalid-request.json');
      const validationResponse = {
        valid: false,
        errors: [
          {
    ...
    it('does not update invalid userId or contentId', async function () {
      const request = require('../fixture/update-rating-request.json');
      const searchResponse = require('../fixture/aws/dynamodb-query-empty-item.json');
      ...
      await rating.updateRating(
        request,
        (err, response) => {
          expect(err.description).to.equal('Invalid userId or contentId');
          expect(err.code).to.equal(400);
        });
    });
  });
  ...
  describe('deleteRating', function () {
    ...
    it('does not delete invalid userId or contentId', async function () {
      const request = require('../fixture/update-rating-request.json');
      const searchResponse = require('../fixture/aws/dynamodb-query-empty-item.json');
