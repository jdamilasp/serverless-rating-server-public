...
/**
 * Get Rating Collection Id for given contentId and userId
 *
 * @param contentId
 * @param userId
 *
 ...
 */
async function getRatingCollectionId(contentId, userId) {
  const searchParams = {
    KeyConditionExpression: 'contentId = :c',
    ExpressionAttributeValues: {
      ':c': {
  ...
  if (response.Count === 0) {
    throw new customError('Invalid userId or contentId');
  }

  const ratingCollectionId = response.Items[0].id.S;

  return ratingCollectionId;
}
...
async function updateRating(request, callback) {
  try {
    ...
    const ratingCollectionId = await getRatingCollectionId(contentId, userId);
